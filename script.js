...
function createWorld() {
  ...
  //scene.fog = new THREE.Fog(Theme._dark, 8, 20);
...
function createGUI() {
  ...
  camGUI
    .add(camera.position, 'z', 3, 20)
    .name('Zoom')
    .listen();
  camGUI
    .add(options.perlin, 'vel', 0.0, 0.02)
    .name('Velocity')
    .listen();
  //camGUI.open();

  var mathGUI = gui.addFolder('Waves');
  mathGUI
    .add(options.spin, 'sinVel', 0.0, 0.5)
    .name('Sine')
    .listen();
  mathGUI
    .add(options.spin, 'ampVel', 0.0, 90.0)
    .name('Amplitude')
    .listen();
  //mathGUI.open();

  var perlinGUI = gui.addFolder('Noise');
  perlinGUI
    .add(options.perlin, 'perlins', 1.0, 5.0)
    .name('Size')
    .step(1);
  perlinGUI
    .add(options.perlin, 'speed', 0.0, 0.0005)
    .name('Speed')
    .listen();
  perlinGUI
    .add(options.perlin, 'decay', 0.0, 1.0)
    .name('Decay')
    .listen();
  perlinGUI
  ...
  perlinGUI
    .add(options.perlin, 'eqcolor', 0.0, 15.0)
    .name('Hue')
    .listen();
  perlinGUI.open();
}
